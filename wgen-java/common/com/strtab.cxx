...
static inline mUINT8 STR_INDEX_size(STR_INDEX idx) { return idx & 0xff; }
static inline STR_INDEX STR_INDEX_index(STR_INDEX idx) { return idx >> 8; }

#endif /* linux */

static inline STR_INDEX make_STR_INDEX(UINT32 size, STR_INDEX idx) {
...
struct NULL_TERMINATED_STRING {
  static char *get_str(char *buffer) { return buffer; }

  static const char *get_str(const char *buffer) { return buffer; }

  static UINT32 get_length(const char *buffer) { return strlen(buffer); }
...
template <class STR> struct STR_TAB {
  char *buffer;
  STR_IDX last_idx;
  ...
  struct extract_key {
    ...
    hash_key operator()(STR_INDEX str_index) const {
      ...
      if (STR_INDEX_size(str_index) < 0xff) {
        return hash_key(STR::get_str(str), STR_INDEX_size(str_index));
      } else {
  ...
  struct equal {
    BOOL operator()(const hash_key &key1, const hash_key &key2) const {
      return (key1.size == key2.size && key1.str[0] == key2.str[0] &&
              memcmp(key1.str, key2.str, key1.size) == 0);
    }
  };

  typedef __gnu_cxx::hashtable<STR_INDEX, hash_key, hash, extract_key, equal>
      HASHTABLE;
  ...
  STR_TAB(UINT bucket_size)
      : hash_table(bucket_size, hash(), equal(), extract_key(*this)),
  ...
  void reserve(UINT32 size) {
    if (size > buffer_size - last_idx) {
      ...
      buffer =
          (char *)MEM_POOL_Realloc(Malloc_Mem_Pool, buffer, 0, buffer_size);
    }
  }
  ...
  STR_INDEX insert(const char *str) { return insert(str, strlen(str)); }
...
template <class STR>
STR_INDEX STR_TAB<STR>::insert(const char *str, UINT32 size) {
...
template <class STR> void STR_TAB<STR>::copy_str(const char *str, UINT32 size) {
  UINT32 buffer_size = STR::get_buffer_length(size);
#ifdef KEY // str may be pointing to buffer, which could be freed in reserve
           // (bug 625)
...
template <class STR> void STR_TAB<STR>::init_hash() {
...
template <class STRTAB>
static inline void initialize_strtab(STRTAB &strtab, const char *buf,
                                     UINT32 size) {
  if (strtab.hash_table.size() != 0)
    strtab.hash_table.erase(strtab.hash_table.begin(), strtab.hash_table.end());
  strtab.buffer_size = size;
  strtab.buffer = (char *)MEM_POOL_Alloc(Malloc_Mem_Pool, size);
...
STR_IDX
STR_Table_Size() { return Strtab.last_idx; }

// init an empty table; use by producer (e.g., front end)

void Initialize_Strtab(UINT32 size) { initialize_strtab(Strtab, size); }
...
UINT32
TCON_strtab_size() { return TCON_strtab.last_idx; }

char *TCON_strtab_buffer() { return TCON_strtab.buffer; }
...
void merge_strtab(STR_TAB<STR> &strtab, const char *buf, UINT32 size,
                  MAP &map) {
  map[0] = 0;
  UINT32 idx = 1;
  while (idx < size) {
